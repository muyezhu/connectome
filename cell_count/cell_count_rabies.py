...
def watershed(gray_img, color_img):
	...
	# save images. for debugging , not needed in production
...
def draw_cell(img, cell_index):
    ...
    for key in cell_index.keys():
        radius = np.int(np.sqrt(cell_index[key][0] / np.pi)) * 2
        cv2.circle(reconstruct, cell_index[key][1], radius, (255, 49, 49), thickness=-1)
...
def cell_count_watershed(gray, original):
	...
	# warped image: size_treshold = 2 * ...   raw unwarped image: size_threshold = 3 * ...
    size_threshold = 2 * mean_dist_transform * mean_dist_transform
